...
func BroadcastTxSync(ctx *rpctypes.Context, tx types.Tx) (*ctypes.ResultBroadcastTx, error) {
	startTime := time.Now()
	env.Logger.Error(fmt.Sprintf("BroadcastTxSync (tendermint) Start for %s:  %s", hex.EncodeToString(tx.Hash()[:]), startTime.String()))
	...
	case res := <-resCh:
		...
		env.Logger.Error(fmt.Sprintf("BroadcastTxSync (tendermint) latency: %s", elapsedTime.String()))
...
func UnconfirmedTxs(ctx *rpctypes.Context, limitPtr *int) (*ctypes.ResultUnconfirmedTxs, error) {
	...
	limit := -1 //validatePerPage(limitPtr)
